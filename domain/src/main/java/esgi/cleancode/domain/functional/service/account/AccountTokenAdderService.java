...
public class AccountTokenAdderService {
    ...
    public Either<ApplicationError, Account> addToken(UUID accountId, int amount) {
        return spi.findById(accountId)
                ...
                .fold(
                        ...
                        account -> verifyAddAndSave(account, amount)
                );
    }

    private Either<ApplicationError, Account> verifyAddAndSave(Account account, int amount) {
